...
class CalculateMealNutrients(
    ...
    operator fun invoke(trackedFoods: List<TrackedFood>): Result {
        val allNutrients = trackedFoods.groupBy { it.mealType }.mapValues { entry ->
            val type = entry.key
            val foodsForDate = entry.value
            MealNutrients(
                carbs = foodsForDate.sumOf { it.carbs },
                protein = foodsForDate.sumOf { it.protein },
                fat = foodsForDate.sumOf { it.fat },
                calories = foodsForDate.sumOf { it.calories },
                mealType = type
            )
        }
