...
class CalculateMealNutrientsTest {
    ...
    fun setUp() {
        val preferences = mockk<Preferences>(relaxed = true) //create mock object
        every { //when we call preferences.loadUserInfo() then return UserInfo object
            preferences.loadUserInfo() // the only function that used in CalculateMealNutrients
        } returns UserInfo(
            gender = Gender.Male,
            age = 20,
            weight = 80f,
        ...
        calculateMealNutrients =
            CalculateMealNutrients(preferences) //We can't give SharedPreferences object because we don't have context
    ...
    fun `Calories for breakfast properly calculated`() {
        ...
        val breakfastCalories =
            result.mealNutrients.values.filter { it.mealType is MealType.Breakfast }.sumOf {
                it.calories
            }
        val expectedCalories = trackedFoods.filter { it.mealType is MealType.Breakfast }.sumOf {
            it.calories
        }
